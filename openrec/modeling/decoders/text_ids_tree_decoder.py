...
#             return len(lines) + 4  # add special tokens <pad><sos><eos><unk>
...
#     def _decode_seq(self, memory, h, mem_mask, tgt_ids: torch.Tensor, embed: nn.Module, proj: nn.Module):
...
#         logits_text, _, pad_text = self._decode_seq(memory, h, mem_mask, tgt_text, self.text_embed, self.proj_text)

#         # ids branch
#         max_ids = int(ids_lengths.max().item())
#         tgt_ids = ids_labels[:, : 2 + max_ids]
#         logits_ids, hidden_ids, pad_ids = self._decode_seq(memory, h, mem_mask, tgt_ids, self.ids_embed, self.proj_ids)
...
#         probs_text = greedy(self.max_text_len, self.text_embed, self.proj_text)
#         probs_ids = greedy(self.max_ids_len, self.ids_embed, self.proj_ids)
...
class TextIDSTreeDecoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _infer_vocab_size(path: Optional[str]) -> int:
            if path is None:
                raise ValueError("vocab_path is required to infer vocab size.")
            # count non-empty lines, consistent with encoder
            with open(path, "r", encoding="utf-8") as f:
    ...
    def forward_test(self, x):
        ...
        def greedy(max_len, embed, proj, pos_enc, norm, decoder):
            tgt = torch.full((B, 1), self.bos_id, dtype=torch.long, device=device)
            logits_out = []
            for i in range(max_len + 1):
                L = tgt.size(1)
