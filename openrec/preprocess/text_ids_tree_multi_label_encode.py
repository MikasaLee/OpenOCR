...
class TextIDSTreeMultiLabelEncode(BaseRecLabelEncode):
    """
    输出（训练建议）:
...
if __name__ == "__main__":
    ...
    for label in test_labels:
        ...
        try:
            ...
            print(f"IDS Label IDs: {data_copy['ids_label']}")
            print(f"IDS Length: {data_copy['ids_length']}")
            print(f"Tree Parents Label: {data_copy['tree_parents_label']}")
            
            inv_ids_dict = {v: k for k, v in encoder.ids_dict.items()}
            decoded_ids = [inv_ids_dict.get(idx, f"<{idx}>") for idx in data_copy["ids_label"] if idx != 0]
