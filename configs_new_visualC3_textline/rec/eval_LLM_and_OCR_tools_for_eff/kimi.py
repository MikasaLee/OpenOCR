...
import ocr_tool_efficiency as eff_utils


DEFAULT_API_KEY = os.getenv('ARK_API_KEY', 'sk-7vh8Kjc8xfPv8ijKD65nKRtQJ6UIpS9LvuVtp9CCRGVqSlYu')
DEFAULT_MODEL_NAME = 'kimi-latest'
...
class DoubaoPredictor:
    ...
    def __init__(self, args, cache_path):
        ...
        self.cache = load_prediction_cache(cache_path) if cache_path else {}
        self.client = None
        self.stats = {
            'cache_hits': 0,
            'api_calls': 0,
...
def evaluate_dataset(dataset_name, dataset_path, predictor, output_rows, det_inputs, limit=0):
    ...
    tgt_match = 0
    num_samples = 0
    runtime_stats = eff_utils.init_runtime_stats(dataset_name)
    ...
    try:
        ...
        for sample in reader.iter_samples(dataset_name=dataset_name, limit=limit):
            ...
            try:
                ...
                eff_utils.add_runtime_record(
                    runtime_stats,
                    wall_time_sec=wall_time_sec,
                    engine_time_sec=latency_sec,
                    cached=cached,
                    status=status,
                )
                ...
                output_rows.append({
                    ...
                    'runtime_measured': bool((not cached) and status == 'ok'),
                    'error': error_message,
                })
    ...
    runtime_summary = eff_utils.finalize_runtime_stats(
        runtime_stats,
        warmup=predictor.args.runtime_warmup,
        max_samples=predictor.args.runtime_max_samples,
    )
...
def parse_args():
    ...
    parser.add_argument(
        '--disable_cache',
        action='store_true',
        help='Disable prediction cache so runtime statistics reflect fresh API calls only.',
    )
    parser.add_argument(
        '--limit',
        type=int,
        default=DEFAULT_LIMIT,
        help='Evaluate at most this many samples for each dataset. Default 120 for slow API runtime runs. 0 means all samples.',
    )
    parser.add_argument(
        '--max_retries',
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help='Discard the first N uncached successful samples when summarizing runtime.',
    )
    parser.add_argument(
        '--runtime_max_samples',
        type=int,
        default=100,
        help='Use at most this many uncached successful samples after warmup. <=0 means all remaining.',
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
...
def main():
    ...
    cache_path = None if args.disable_cache else (
        args.cache_path or os.path.join(output_dir, 'pred_cache.jsonl')
    )
    ...
    for dataset_name, dataset_path in build_dataset_jobs(args):
        ...
        runtime_summaries.append(summary.get('runtime'))
        print_summary(summary)
        eff_utils.print_runtime_summary(summary['runtime'])
        eff_utils.print_runtime_table_summary(summary['runtime'])

    overall_summary = summarize_rows('total (text)', output_rows)
    overall_runtime = eff_utils.aggregate_runtime_summaries(
        runtime_summaries,
        dataset_name='total',
    )
    print_summary(overall_summary)
    eff_utils.print_runtime_summary(overall_runtime)
    eff_utils.print_runtime_table_summary(overall_runtime)

    if predictor.stats['cache_hits'] > 0 and not args.disable_cache:
        print('[WARN] Runtime summary excludes cached samples. Use --disable_cache for reproducible efficiency numbers.')
    ...
    stats_lines = [
        ...
        f'cache_path: {"DISABLED" if cache_path is None else cache_path}',
        f'disable_cache: {args.disable_cache}',
        ...
        f'runtime_warmup: {args.runtime_warmup}',
        f'runtime_max_samples: {args.runtime_max_samples}',
        '',
    ]
    ...
    for summary in dataset_summaries:
        ...
        stats_lines.append(
            f"{summary['dataset_name']} runtime: "
            f"avg_runtime={fmt(runtime.get('avg_runtime_sec'))} s/line, "
            f"p50={fmt(runtime.get('p50_runtime_sec'))} s, "
            f"p90={fmt(runtime.get('p90_runtime_sec'))} s, "
            f"measured={runtime.get('num_measured', 0)}, "
            f"warmup={runtime.get('warmup')}, "
            f"cached={runtime.get('num_cached', 0)}"
        )
    ...
    stats_lines.extend([
        ...
        (
            f"total runtime: avg_runtime={fmt(overall_runtime.get('avg_runtime_sec'))} s/line, "
            f"throughput={fmt(overall_runtime.get('throughput_lines_per_sec'))} lines/s, "
            f"measured={overall_runtime.get('num_measured', 0)}, "
            f"warmup={overall_runtime.get('warmup')}, "
            f"cached={overall_runtime.get('num_cached', 0)}"
        ),
        '',
    ])
