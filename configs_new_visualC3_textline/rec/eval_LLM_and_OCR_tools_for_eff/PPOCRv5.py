...
DEFAULT_MODEL_NAME = 'ppocrv5_server'
DEFAULT_FAILED_PRED = '#'
...
def parse_args():
    ...
    parser.add_argument(
        '--disable_cache',
        action='store_true',
        help='Disable prediction cache so runtime statistics reflect fresh calls only.',
    )
    parser.add_argument(
        '--limit',
        type=int,
        default=DEFAULT_LIMIT,
        help='Evaluate at most this many samples for each dataset. Default 120 for slow OCR-tool runtime runs. 0 means all samples.',
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
...
def main():
    ...
    overall_runtime = eff_utils.aggregate_runtime_summaries(runtime_summaries, dataset_name='total')
