...
def char_edit_counts(pred_text, gt_text):
    """Return character-level substitutions, deletions, insertions, and GT length."""
    pred_chars = list(pred_text or '')
    gt_chars = list(gt_text or '')
    pred_len = len(pred_chars)
    gt_len = len(gt_chars)

    # dp[i][j] stores (edit_cost, substitutions, deletions, insertions)
    # for converting gt_chars[:j] into pred_chars[:i].
    dp = [[(0, 0, 0, 0) for _ in range(gt_len + 1)] for _ in range(pred_len + 1)]
    ...
    for i in range(1, pred_len + 1):
        for j in range(1, gt_len + 1):
            prev_cost, prev_subs, prev_dels, prev_ins = dp[i - 1][j - 1]
            if pred_chars[i - 1] == gt_chars[j - 1]:
                candidates = [(prev_cost, prev_subs, prev_dels, prev_ins)]
            else:
                candidates = [(prev_cost + 1, prev_subs + 1, prev_dels, prev_ins)]

            ins_cost, ins_subs, ins_dels, ins_ins = dp[i - 1][j]
            candidates.append((ins_cost + 1, ins_subs, ins_dels, ins_ins + 1))

            del_cost, del_subs, del_dels, del_ins = dp[i][j - 1]
            candidates.append((del_cost + 1, del_subs, del_dels + 1, del_ins))
    ...
    _, substitutions, deletions, insertions = dp[pred_len][gt_len]
    return substitutions, deletions, insertions, gt_len


def init_hctr_error_stats():
    return {
        'substitutions': 0,
        'deletions': 0,
        'insertions': 0,
...
def update_hctr_error_stats(stats, pred_text, gt_text):
    substitutions, deletions, insertions, char_count = char_edit_counts(pred_text, gt_text)
    ...
    return finalize_hctr_error_stats(sample_stats)


def merge_hctr_error_stats(total_stats, stats):
    if total_stats is None:
        total_stats = init_hctr_error_stats()
    for key in total_stats:
        total_stats[key] += stats.get(key, 0)
    return total_stats


def finalize_hctr_error_stats(stats):
    ...
    if char_count == 0:
        clean = edit_errors == 0
        return {
            'cr': 1.0 if clean else 0.0,
            'cer': 0.0 if clean else 1.0,
            'ar': 1.0 if clean else 0.0,
        }

    return {
        'cr': (char_count - deletions - substitutions) / char_count,
        'cer': edit_errors / char_count,
        'ar': (char_count - deletions - substitutions - insertions) / char_count,
    }
...
def dump_predictions(trainer, datadir, output_log, dataset_name, image_bytes, infer_branch='ssm'):
    ...
    true_num = 0
    ned_list = []
    hctr_stats = init_hctr_error_stats()
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(valid_dataloader):
            ...
            for i, (txt, prob) in enumerate(texts):
                ...
                hctr_metrics = update_hctr_error_stats(hctr_stats, txt_norm, gt_norm)
                ned_list.append(ned)
                num += 1
                if int(ned) == 1:
                    true_num += 1
                ...
                output_log['CR'].append(float(hctr_metrics['cr']))
                output_log['CER'].append(float(hctr_metrics['cer']))
                output_log['AR'].append(float(hctr_metrics['ar']))
                try:
    ...
    return pnacc, ned_mean, num, hctr_stats
...
def dump_predictions_with_efficiency(trainer, datadir, output_log, dataset_name, image_bytes, infer_branch='ssm'):
        ...
        for _batch_idx in range(len(valid_dataloader)):
            ...
            for i, (txt, prob) in enumerate(texts):
                ...
                sample_records.append({
                    ...
                    'pred': txt_norm,
                    'NED': float(ned),
                    'CR': float(hctr_metrics['cr']),
                    'CER': float(hctr_metrics['cer']),
                    'AR': float(hctr_metrics['ar']),
                    'sample_img': raw_images[i] if raw_images is not None else None,
                })
            ...
            for record in sample_records:
                ...
                output_log['CR'].append(record['CR'])
                output_log['CER'].append(record['CER'])
                output_log['AR'].append(record['AR'])
                try:
    ...
    pnacc = true_num / num if num else 0.0
    ned_mean = float(np.mean(ned_list)) if ned_list else 0.0
    return pnacc, ned_mean, num, eff_stats, hctr_stats
...
def main():
    ...
    data_dirs_list = [[
        ...
        r'/a800data1/lirunrui/lmdb_output/test/IC13Comp_line_lmdb/',
    ]]
    ...
    for data_dirs in data_dirs_list:
        for datadir in data_dirs:
            ...
            hctr_metrics = finalize_hctr_error_stats(hctr_stats)
            print(
                f"{dataset_name}:\t\t acc: {100 * pnacc:6g}, "
                f"norm_edit_dis:{100 * ned_mean:6g}, "
                f"CR:{100 * hctr_metrics['cr']:6g}, "
                f"CER:{100 * hctr_metrics['cer']:6g}, "
                f"AR:{100 * hctr_metrics['ar']:6g}"
            )
            print_efficiency_summary(finalize_efficiency_stats(eff_stats))
            every_PNacc_list.append(pnacc)
            every_ned_list.append(ned_mean)
            every_cr_list.append(hctr_metrics['cr'])
            every_cer_list.append(hctr_metrics['cer'])
            every_ar_list.append(hctr_metrics['ar'])
            total_num += num
            ...
            total_hctr_stats = merge_hctr_error_stats(total_hctr_stats, hctr_stats)
    ...
    try:
        ...
        total_hctr_metrics = finalize_hctr_error_stats(total_hctr_stats or init_hctr_error_stats())
        total_cr = total_hctr_metrics['cr']
        total_cer = total_hctr_metrics['cer']
        total_ar = total_hctr_metrics['ar']
        ...
        s_mean_cr = float(np.mean(every_cr_list)) if every_cr_list else 0.0
        s_mean_cer = float(np.mean(every_cer_list)) if every_cer_list else 0.0
        s_mean_ar = float(np.mean(every_ar_list)) if every_ar_list else 0.0
        if every_PNacc_list:
            acc_arr = np.array(every_PNacc_list, dtype=np.float32)
            ned_arr = np.array(every_ned_list, dtype=np.float32)
            cr_arr = np.array(every_cr_list, dtype=np.float32)
            cer_arr = np.array(every_cer_list, dtype=np.float32)
            ar_arr = np.array(every_ar_list, dtype=np.float32)
            if len(acc_arr) == len(S_WEIGHT):
                ...
                s_weight_cr = float(np.sum(cr_arr * weights))
                s_weight_cer = float(np.sum(cer_arr * weights))
                s_weight_ar = float(np.sum(ar_arr * weights))
            else:
                ...
                s_weight_cr = s_mean_cr
                s_weight_cer = s_mean_cer
                s_weight_ar = s_mean_ar
        else:
            s_weight_acc = 0.0
            s_weight_ned = 0.0
            s_weight_cr = 0.0
        ...
        print(
            ...
            f"CR:{100 * total_cr:6g}, CER:{100 * total_cer:6g}, AR:{100 * total_ar:6g}"
        )
        print(
            f"S_mean:\t\t acc: {100 * s_mean_acc:6g}, "
            f"norm_edit_dis:{100 * s_mean_ned:6g}, "
            f"CR:{100 * s_mean_cr:6g}, CER:{100 * s_mean_cer:6g}, AR:{100 * s_mean_ar:6g}"
        )
        print(
            f"S_weight:\t\t acc: {100 * s_weight_acc:6g}, "
            f"norm_edit_dis:{100 * s_weight_ned:6g}, "
            f"CR:{100 * s_weight_cr:6g}, CER:{100 * s_weight_cer:6g}, AR:{100 * s_weight_ar:6g}"
        )
        if total_eff_stats is not None:
            print_efficiency_summary(finalize_efficiency_stats(total_eff_stats))
        print(f'Predictions (with NED/CR/CER/AR) saved to {save_pred_xlsx}')
