...
def prepare_cfg(cfg):
    ...
    cfg.cfg['Metric']['max_len'] = 30 # 10+3+10。
    cfg.cfg['Global']['max_text_length'] = 30 # 10+3+10。
        ...
        for batch_idx, batch in enumerate(valid_dataloader):
            ...
            for i, (txt, prob) in enumerate(texts):
                ...
                if gts is not None and i < len(gts):
                    ...
                    gt_text = gts[i][0]
                gt_src_txt,gt_tgt_txt = gt_text.split('|||')
                # print(f"GT_src:{gt_src_txt}\tGT_tgt:{gt_tgt_txt}\tGT:{gt_text}") # debug
                # 标点标准化后再计算 NED
...
def main():
    ...
    for data_dirs in data_dirs_list:
        for datadir in data_dirs:
            ...
            src_pnacc, tgt_pnacc, src_ned_mean, tgt_ned_mean, num = dump_predictions(trainer, datadir, output_log, dataset_name)
